...
const Landing = () => {
  ...
  const activeData = dataCompetition.find((data) => data.category === activeTab) || dataCompetition[0];

  return (
    <section className='w-full min-h-auto flex flex-col self-stretch items-start justify-start relative gap-5'>
        <Navbar/>
        <Image
          src={"/image 44.webp"}
          ...
          width={500}
          height={500}
          className='w-80.5 sm:w-62.5 md:w-100 lg:w-200 absolute object-cover top-0 right-0 z-10'
        />
            ...
            <div className='hidden md:flex flex-row items-center justify-start gap-4'>
              {dataCompetition.map((data) => (
                <button 
                  ...
                  className={`flex-none w-37.5 py-2 items-center justify-center rounded-[30px] font-montserrat font-medium text-md transition-colors cursor-pointer ${
                    activeTab === data.category
                      ? "bg-[linear-gradient(90deg,#FBD596_0%,#FBC364_100%)] text-[#0E1527] shadow-[0_4px_4px_rgba(0,0,0,0.15)]"
                      : "bg-[rgba(255_255_255_0.10)] bacdrop-blur-md text-white "
                  }`}
                >
                  {data.category}
                </button>
        ...
        <div className='w-full flex items-center justify-center relative h-auto z-10 px-4 md:px-12 pb-12 overflow-hidden'>
          <div className='mx-auto w-full md:max-w-6xl rounded-3xl sm:rounded-4xl md:rounded-[40px] lg:rounded-[48px] border-b-2 border-r border-l border-white bg-white/10 backdrop-blur-md box-border shadow-[0_4px_4px_rgba(0,0,0,0.25)] px-6 sm:px-8 md:px-10 lg:px-12 py-8 sm:py-10 md:py-12 flex items-start justify-center flex-col gap-4 md:gap-6 transition-all duration-500 ease-in-out'>
            <h1 className='text-left font-playfair font-semibold bg-clip-text text-transparent bg-[linear-gradient(102deg,#FBD596_0.77%,#FBC364_22.3%,#FBD596_41.41%,#FBC364_65.41%,#FBD596_87.21%,rgba(251,195,100,0.75)_107.16%)] text-2xl md:text-3xl lg:text-4xl '>{activeData.title}</h1>
            <p className='text-justify font-montserrat text-sm md:text-base lg:text-lg font-normal leading-normal tracking-[-0.3px] text-white'>{activeData.description}</p>
            <div className='w-full flex flex-col md:flex-row-reverse items-center justify-center md:justify-start gap-4 px-0 md:px-5 mt-4'>
              {dataCompetition.map((dataCompetition, index) => (
                activeData.id === dataCompetition.id && (
                  <React.Fragment key={index}>
                    <Link className='w-full md:w-auto rounded-full flex items-center justify-center px-8 py-3 bg-[linear-gradient(90deg,#47427C_-10.14%,#9A4183_53.2%,#E18BA6_116.55%)] text-white text-center font-montserrat text-sm md:text-base font-semibold whitespace-nowrap shadow-md cursor-pointer hover:font-bold transition-all duration-300 ease-in-out' href={dataCompetition.LinkPendaftaran}>
                        SUBMITION
                    </Link>
                    <Link
                      className="w-full md:w-auto relative px-8 py-3 rounded-full text-white font-bold tracking-widest text-xs md:text-sm bg-transparent"
                      style={{
                    ...
                    >
                      <span
                        ...
                        style={{
                          padding: "2px",
                          background: "linear-gradient(90deg, #47427C, #9A4183, #E18BA6)",
